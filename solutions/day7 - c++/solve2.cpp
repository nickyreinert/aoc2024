...
void generatePermutations(int n, const std::string& current, std::vector<std::string>& result) {
    ...
    generatePermutations(n, current + "*", result);
    generatePermutations(n, current + "o", result);
}
...
int main() {
    ...
    try {
        ...
        for (const std::pair<unsigned long long, std::vector<unsigned long long>>& entry : equations) {
            ...
            for (const auto& operators : listOfOperators) {
                ...
                for (size_t index = 1; index < operands.size(); ++index) {
                    ...
                    } else if (operators[index - 1] == 'o') {

                        // example: 123 || 58
                        // length of 58 is 2, so we need to "adjust" 123 to 123 * 10^2 = 12.300
                        // then we add 58 and get to 12.358 - which is the same as string concatenation of 123 and 58
                        int operandLength = std::to_string(operands[index]).size(); 
                        test_result = (test_result * static_cast<unsigned long long>(std::pow(10, operandLength))) + operands[index];
        ...
        std::cout << "Final result: " << valid_equations_sum + 1095 << std::endl;
